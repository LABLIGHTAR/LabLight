...
// {
    public class ProtocolView : VisualElement
    {
        ...
        // Method to reset UI elements related to a specific step
        private void ResetStepUIData(string stepIndicatorText = "Step - / -")
        {
            ...
            // _isSignedOff should be managed by OnStepChanged based on new step state
            // UpdateSignOffButtonState(); // This will be called after _isSignedOff is set
            ResetSignatureLine();
        }
        ...
        private void PopulateChecklist(List<ProtocolState.CheckItemState> checklistItemStatesList)
        {
            ...
            for (int i = 0; i < checklistItemStatesList.Count; i++)
            {
                ...
                // Apply dynamic styling based on interaction logic
                if (_isSignedOff)
                {
                ...
                {
                    bool isTheNextItemToBeChecked = (i == firstUncheckedIndex && firstUncheckedIndex != -1); // Ensure firstUncheckedIndex is valid
                    bool canThisItemBeUnchecked = itemState.IsChecked.Value && 
                                                ((firstUncheckedIndex == -1 && i == checklistItemStatesList.Count - 1) || // Last item, all checked
                                                 (firstUncheckedIndex > 0 && i == firstUncheckedIndex - 1));      // Item before first unchecked

                    if (isTheNextItemToBeChecked)
                    {
                        checkItemVisual.AddToClassList("check-item-next");
                    }
                    // If it's not the next item, and not one of the special uncheckable items, then it's locked.
                    else if (!canThisItemBeUnchecked) 
                    {
                        checkItemVisual.AddToClassList("check-item-locked");
                    }
                    // If it's an 'uncheckablePrevious' or 'uncheckableLastWhenAllDone', it's interactable,
                    // so no 'locked' or 'next' class applies (it will have 'completed' if checked).
                }
        ...
        private void OnProtocolChanged(ProtocolDefinition protocol)
        {
            ...
            else
            {
                ...
                ResetStepUIData(); // Use consolidated reset method
                _isSignedOff = false; // Ensure signed off state is reset
                UpdateSignOffButtonState(); // Update button based on reset state
            }
        }
        ...
        private void OnStepChanged(int stepIndex)
        {
            ...
            {
                Debug.LogWarning($"[ProtocolView] Invalid step index: {stepIndex} for protocol '{_currentProtocol?.title}'. Steps available: {_currentProtocol?.steps?.Count ?? 0}. Clearing step view.");
                ...
                _isSignedOff = false; // Reset signed-off state for invalid step
                UpdateSignOffButtonState();
                return;
            }
            
            // Valid step, proceed with fetching step definition and state
        ...
        private void UpdateSignOffButtonState()
        {
            ...
            {
                // If no step state, no checklist, or empty checklist, consider it "all checked" for sign-off purposes.
            ...
            // _isSignedOff should be the authoritative value for the current step's signed-off status.
            if (_isSignedOff)
            {
                _signOffActionButton.SetEnabled(false);
                // Optionally, visually indicate signed-off state on the button itself
                // Example: _signOffActionButton.AddToClassList("signed-off"); 
            }
            else
            {
                _signOffActionButton.SetEnabled(allItemsEffectivelyChecked);
                // Example: _signOffActionButton.RemoveFromClassList("signed-off");
            }
        }
